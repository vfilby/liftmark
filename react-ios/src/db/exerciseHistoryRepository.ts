...
// ============================================================================
// Normalized Variants (merge aliases by canonical name)
// ============================================================================

/**
 * Build a SQL IN-clause fragment and params for all aliases of an exercise name.
 */
function aliasInClause(name: string): { placeholders: string; params: string[] } {
  const aliases = getAliases(name);
  return {
    placeholders: aliases.map(() => '?').join(','),
    params: aliases,
  };
}

/**
 * Get exercise history across all aliases of the given exercise name.
 */
export async function getExerciseHistoryNormalized(
  exerciseName: string,
  limit: number = 10
): Promise<ExerciseHistoryPoint[]> {
  const db = await getDatabase();
  const { placeholders, params } = aliasInClause(exerciseName);
  ...
  const rows = await db.getAllAsync<{
    ...
    WHERE ws.status = 'completed'
      AND ss.status = 'completed'
      AND LOWER(se.exercise_name) IN (${placeholders})
    GROUP BY ws.id, ws.date
    ORDER BY ws.date DESC
    LIMIT ?
  `, [...params, limit]);
  ...
  return rows
    ...
    .map(row => ({
      ...
      workoutName: row.workout_name,
      maxWeight: row.max_weight ?? 0,
      avgReps: Math.round((row.avg_reps ?? 0) * 10) / 10,
      totalVolume: Math.round(row.total_volume ?? 0),
      setsCount: row.sets_count,
      avgTime: Math.round(row.avg_time ?? 0),
      maxTime: Math.round(row.max_time ?? 0),
...
/**
 * Get session-by-session history across all aliases of the given exercise name.
 */
export async function getExerciseSessionHistoryNormalized(
  exerciseName: string,
  limit: number = 30
): Promise<ExerciseSessionData[]> {
  const db = await getDatabase();
...
/**
 * Get progress metrics across all aliases of the given exercise name.
 */
export async function getExerciseProgressMetricsNormalized(
  exerciseName: string
): Promise<ExerciseProgressMetrics | null> {
  const db = await getDatabase();
  const { placeholders, params } = aliasInClause(exerciseName);
...
/**
 * Get all exercises that have been performed, deduplicated by canonical name.
 * Returns canonical names where possible, original names for unknowns.
 */
export async function getAllExercisesWithHistoryNormalized(): Promise<string[]> {
  const raw = await getAllExercisesWithHistory();
