...
8. **Rest timer**: `startRestTimer()` begins after set completion, counts down to zero, and auto-stops.
9. **Exercise timer**: For time-based sets, tracks elapsed time versus target duration.
...
- **Time-based sets**: The exercise timer tracks elapsed time against a target rather than counting reps.
